...
The helper script `scripts/build_disk_image.sh` assembles both boot paths into `.bin/casseos.img` by placing the BIOS boot sector + kernel at the front of the disk while reserving a 64 MB FAT32 ESP that carries the freshly built `EFI/BOOT/BOOTX64.EFI`. The UEFI loader now locates `\CASSEKRN.BIN` on the ESP, loads it to the same physical base as the BIOS path, validates a small boot-info header embedded at the start of the kernel image, exits boot services, and jumps into the kernel's dedicated UEFI entry point. A `startup.nsh` script is also dropped at the ESP root; on the first boot it registers a persistent `Boot####` entry via `bcfg` and chains into `\EFI\BOOT\BOOTX64.EFI`, so subsequent boots jump straight into the loader with no shell countdown (you can still hit `Esc` during the prompt or use the OVMF Boot Manager to reach the shell when needed).
